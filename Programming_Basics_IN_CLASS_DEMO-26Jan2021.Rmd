...
rbi_NT$STANAME <- as.character(rbi_NT$STANAME)
...
filter(rbi, STATE %in% statesIwant)
```
...
```{r}
summarize(rbi, meanRBI = mean(RBI), minRBI = min(RBI), maxRBI = max(RBI))
...
summarize(rbistate, meanRBI = mean(RBI), minRBI = min(RBI), maxRBI = max(RBI))
```
...
```{r}
rbi %>% group_by(STATE) %>%
summarize(meanRBI = mean(RBI), minRBI = min(RBI), maxRBI = max(RBI))
```
...
rbi %>% group_by(STATE) %>%
summarize(meanRBI = mean(RBI), minRBI = min(RBI), maxRBI = max(RBI)) -> sumrbi
